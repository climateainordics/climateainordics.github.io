...
{% if p.deadline %}{% if deadlinetime < oneweektime %}<span style="font-weight: bold; color: #f00;">{% endif %}Deadline: {{ p.deadline }}{% if deadlinetime < oneweektime %}</span>{% endif %}<br />{% endif %}
